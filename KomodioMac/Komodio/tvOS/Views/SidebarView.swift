    ...
    var body: some View {
        VStack(alignment: .leading) {
            Label(
                title: {
                    VStack(alignment: .leading) {
                        Text(appState.host?.description ?? "No host selected")
                    ...
                    Image(systemName: "sparkles.tv")
                        .foregroundColor(scene.mainSelection == 0 ? .blue : .gray)
        ...
        Label(
            title: {
                ...
                Image(systemName: item.label.icon)
                    .foregroundColor(scene.sidebarItems[selection] == item ? .blue : .gray)
